...
export class CanvasApp {
  ...
  private setCameraOffset: React.Dispatch<
      ...
      if (message.type === "draw") {
        ...
        this.existingShapes = this.existingShapes.map((shapes) => {
          if (
            ...
            shapes.Rectangle?.id === shape.Rectangle.id
          ) {
            return { ...shapes, Rectangle: shape.Rectangle };
          } else if (
            shape.type === "Circle" &&
            shapes.Circle?.id === shape.Circle.id
          ) {
            return { ...shapes, Circle: shape.Circle };
          } else if (
            shape.type === "Line" &&
            shapes.Line?.id === shape.Line.id
          ) {
  ...
  private onMouseDown = (e: MouseEvent) => {
    ...
    if (tool === toolType.select) {
      const result = handleShapeSelection(
        worldMouseX,
        worldMouseY,
        this.existingShapes,
        this.canvas,
      ...
      this.render();
        ...
        this.ctx.strokeText("Hello world", this.startX, this.startY);
    }
  };
  ...
  private onMouseUp = (e: MouseEvent) => {
    ...
    const newShape = handleShapeDraw(
      tool,
      this.startX,
      this.startY,
      worldMouseX,
